background = {
    ...
    init: function() {
		...
		chrome.runtime.onMessageExternal.addListener( function(request, sender, callBack) {
			...
			{
				case "login":
		        	app.login(request.userKey, request.userVal, request.installKey, request.installVal)
		        	return true;
		        	break;
	        }
			return false;
...
app = {
	login: function(userKey, userVal, installKey, installVal) {
...
app.Recipe = Parse.Object.extend("Recipe", {
	...
	uploadImage: function(url) {
		app.utils.uploadWebImageToAmazon(url, 'users/'+app.loginedUser.id+'/recipes/'+this.id+'/cover',function(){});
	}
})
	...
	uploadWebImageToAmazon: function(imgUrl, toPath, callback) {
		...
		getImg.onload = function(oEvent) {
			...
			Parse.Cloud.run('awsInfo', {file: toPath, fileType: "image/jpg"}).then(function(result) {
			    ...
			    fd.append('AWSAccessKeyId', background.settings.AWSAccessKeyId);
			    ...
			    xhr.open('POST', 'https://'+background.settings.amazonBucket+'.s3.amazonaws.com/', true); //MUST BE LAST LINE BEFORE YOU SEND 
			    xhr.send(fd);
			})
		}
		getImg.send();
